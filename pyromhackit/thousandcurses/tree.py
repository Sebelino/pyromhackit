...
class Tree(treelib.Tree):
    ...
    def from_nested_container(self, arg):
        if not self.is_treelike(arg) or not self.type(arg) in {str, bytes}:
            raise ValueError(
    ...
    @staticmethod
    def type(tipe, pseudotree):
        """ Returns the single type that every non-iterable element in the treelike structure shares, or None if there
        exist elements with different types. """
        container_types = {tuple, list, Tree}
        content_types = {bytes, str}
        q = Queue()
        q.put(pseudotree)
